...
class PaginatedJobsResponse(BaseModel):
    """API response for historical paginated jobs (offset-based)."""
    data: list[JobListing]
    total: int
    offset: int
    limit: int
    has_more: bool
    # kept for backwards compat with callers that read .page
    page: int = 1
...
class JobDocument(BaseModel):
    ...
    title: Optional[str] = None
    title_norm: Optional[str] = None         # lowercase + stripped, for aggregation
    company: Optional[str] = None
    location: Optional[str] = None
    location_norm: Optional[str] = None      # lowercase + stripped, for aggregation
    job_type: Optional[str] = None
    is_remote: Optional[bool] = None
    date_posted: Optional[str] = None
    job_url: Optional[str] = None
